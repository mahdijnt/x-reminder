...
from handlers.conversations.account_flows import (
    add_username_received,
    cancel_conversation,
    remove_username_received,
)
...
def build_application(settings: Settings, api_client: ApiClient | None = None) -> Application:
    ...
    add_conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(
                filters.Regex(f"^{t('reply.add_account')}$"),
                add_command,
            ),
        ],
        states={
            STATE_ADD_USERNAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, add_username_received)
            ],
        },
    ...
    remove_conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(
                filters.Regex(f"^{t('reply.remove_account')}$"),
                remove_command,
            ),
        ],
        states={
            STATE_REMOVE_USERNAME: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, remove_username_received)
            ],
        },
        fallbacks=[CommandHandler("cancel", cancel_conversation)],
        name="remove_account",
    ...
    reply_labels = "|".join(
        [
            t("reply.dashboard"),
            t("reply.my_lists"),
            t("reply.target_achieved"),
            t("reply.settings"),
        ]
    )
    application.add_handler(
        MessageHandler(filters.Regex(f"^({reply_labels})$"), reply_menu_router)
    )
